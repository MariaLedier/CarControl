...
export default function AbastecimentoVeiculoPage() {
    ...
    function formatarDataInput(data) {
        if (!data) return ""
        // Pega só os primeiros 10 caracteres (YYYY-MM-DD) sem converter para Date
        // evita problema de fuso horário que muda o dia
    ...
    async function salvar(e) {
        ...
        try {
            ...
            let resultado
            if (editando) {
                resultado = await apiClient.put("/abastecimento", { id: editando.id, ...payload })
            } else {
                resultado = await apiClient.post("/abastecimento", payload)
            }

            // Se o apiClient não retornou nada, significa que houve erro (já exibido pelo apiClient)
            if (!resultado) return
