import AuthMiddleware from "../middlewares/authMiddleware.js";
import UsuarioRepository from "../repositories/usuarioRepository.js";

export default class AutenticacaoController {

    #usuarioRepository;

    constructor() {
        this.#usuarioRepository = new UsuarioRepository();
    }

    async usuario(req, res) {
        try {
            if (req.usuarioLogado)
                return res.status(200).json(req.usuarioLogado);
            else
                throw new Error("Erro ao obter o usuário!");
        }
        catch (ex) {
            console.log(ex);
            return res.status(500).json({ msg: "Erro ao obter usuário" })
        }
    }

   async token(req, res) {
    try {
        let { nome, senha } = req.body;

        if (nome && senha) {
            // primeiro verifica se o nome existe
            let usuarioPorNome = await this.#usuarioRepository.buscarPorNome(nome);

            if (!usuarioPorNome) {
                return res.status(400).json({ msg: "Nome de usuário não encontrado" });
            }

            // nome existe — agora verifica a senha
            let usuario = await this.#usuarioRepository.buscarPorLogin(nome, senha);

            if (!usuario) {
                return res.status(400).json({ msg: "Senha incorreta" });
            }

            let auth = new AuthMiddleware();
            let token = auth.gerarToken(usuario.id, usuario.nome, usuario.tipo);

            res.cookie("token", token, {
                httpOnly: true,
                sameSite: "none",
                secure: true,
                maxAge: 8 * 60 * 60 * 1000
            });

            return res.status(200).json({
                token: token,
                usuario: { id: usuario.id, nome: usuario.nome, tipo: usuario.tipo }
            });

        } else {
            return res.status(400).json({ msg: "Informe nome e senha!" });
        }
    }
    catch (exception) {
        console.log(exception);
        return res.status(500).json({ msg: "Erro ao gerar token de acesso" });
    }
}

    async logout(req, res) {
        try {
            res.clearCookie("token", { sameSite: "none", secure: true });
            return res.status(200).json({ msg: "Logout realizado!" });
        } catch (ex) {
            console.log(ex);
            return res.status(500).json({ msg: ex.message });
        }
    }
}